...
@onready var sfx_vol_slider: HSlider = %TurretVolSlider
@onready var sfx_volume_label: Label = %TurretVolumeLabel
...
func _ready() -> void:
	...
	sfx_vol_slider.step = 0.05
	sfx_vol_slider.value = SettingsManager.volume_turret
	...
	sfx_vol_slider.value_changed.connect(func(v): SettingsManager.set_volume("sfx", v) ;sfx_volume_label.text = "Sfx Volume: %d%%" % int(v * 100))
	...
	sfx_volume_label.text = "Sfx Volume: %d%%" % int(SettingsManager.volume_turret * 100)
